...
hpi_tidy <- 
  hpi_wrangled %>% 
  select(date, contains("atlantic"), contains("central")) %>% 
  # pivot_longer makes data long, or tidy
  pivot_longer(-date, names_to = "region", values_to = "hpi") %>% 
  group_by(region)

hpi_untidied <- 
  hpi_tidy %>% 
  # pivot_wider makes data wide, or untidy
  pivot_wider(names_from = "region", values_from = "hpi")
```
...
ggplot(hpi_tidy, aes(x = date, y = hpi, color = region)) +
         geom_line()
```
